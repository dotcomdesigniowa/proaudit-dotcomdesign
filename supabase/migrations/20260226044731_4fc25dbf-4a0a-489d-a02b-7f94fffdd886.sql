...
-- Update auto_create_share_link to extract domain from website_url as slug
...
BEGIN
  ...
  -- Extract domain from website_url for clean slug
  v_domain := COALESCE(NEW.website_url, '');
  -- Strip protocol
  ...
  -- Fallback to company name if no website_url
...
-- Update record_share_view to also support lookup by slug (domain)
...
BEGIN
  ...
  -- Then try slug (domain-based lookup)
  IF NOT FOUND THEN
    SELECT * INTO v_share FROM audit_shares
    WHERE slug = p_share_token
      AND is_active = true
      AND (expires_at IS NULL OR expires_at > now())
