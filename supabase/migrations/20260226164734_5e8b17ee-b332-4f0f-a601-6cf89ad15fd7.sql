...
-- Update scoring trigger: skip design weight when provider is "Other"
...
BEGIN
  ...
  -- Determine if design should be included (excluded for "Other" provider)
  ...
  ELSE
    IF v_include_design THEN
      -- Normal calculation with design weight
    ...
    ELSE
      -- Exclude design: redistribute its weight proportionally among remaining 3
      v_total_weight := v_w_w3c + v_w_psi + v_w_acc;
      NEW.overall_score := ROUND(
